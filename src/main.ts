...
function updateTrayIcon() {
  ...
  try {
    ...
    if (newTheme !== currentThemeName || currentThemeName === '') {
      ...
      if (trayImage.isEmpty() && iconPath.endsWith('.svg')) {
          const pngBuffer = safeBashExecBuffer(`rsvg-convert -w 22 -h 22 "${iconPath}"`);
      ...
      if (trayImage.isEmpty()) {
          console.warn(`[Tray] Icon empty for path "${iconPath}". Falling back to default.`);
